...
async function seed() {
  ...
  for (const user of companyUsers) {
    const company = await db.insert(companies).values({
      ...
      industry: faker.helpers.arrayElement(["TECH", "AGRI", "HEALTH", "FINANCE", "EDUCATION", "OTHER"]),
  ...
  // Create job seekers
  const jobSeekerUsers = createdUsers.filter(user => user.role === "jobseeker");
  ...
  for (const user of jobSeekerUsers) {
    const jobSeeker = await db.insert(jobSeekers).values({
      ...
      preferredJobType: faker.helpers.arrayElement(["REMOTE", "ONSITE", "HYBRID"]),
      availableSchedule: {
      ...
      const job = await db.insert(jobs).values({
        ...
        requiredSkills: Array.from({ length: faker.number.int({ min: 3, max: 8 }) }, () => 
          faker.helpers.arrayElement([
            ...
            "SQL", "AWS", "Docker", "Kubernetes", "GraphQL", "REST API"
          ])
        ),
  ...
  for (const jobSeeker of createdJobSeekers) {
    ...
    const randomJobs = faker.helpers.arrayElements(createdJobs, savedJobCount);
