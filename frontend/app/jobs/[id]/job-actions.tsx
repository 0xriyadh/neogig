...
export function JobActions({ jobId }: JobActionsProps) {
  ...
  const { data: application } = trpc.application.getByJobId.useQuery({ jobId });
  ...
  async function handleWithdrawInterest() {
    ...
    setIsLoading(true);
    try {
      await withdrawInterest.mutateAsync({ applicationId: application[0].id });
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <>
      <Button
        size="lg"
        variant={savedJob ? "secondary" : "default"}
        className="flex-1"
        onClick={handleSaveJob}
        disabled={isLoading}
      >
        <Bookmark className="w-4 h-4 mr-2" />
        {savedJob ? 'Unsave Job' : 'Save Job'}
      </Button>
