...
export function JobList() {
  ...
  const [workType] = useQueryState('workType', {
    defaultValue: '',
  });
  ...
  const { data: jobs, isLoading, error } = useQuery({
    queryKey: ['jobs', selectedJobTypes, experienceLevel, salaryRange, location, workType, selectedSkills, showUrgentOnly],
    queryFn: async () => {
      const data = await fetchTrpc<Job[]>('job.getAll');
      
      const dataWithCompany = await Promise.all(data.map(async (job) => {
        const company = await fetchTrpc<Company>('company.getById', { userId: job.companyId }).catch(() => null);
        return { ...job, company };
      }));
      ...
      return dataWithCompany.filter(job => {
        // Job type filter
        if (selectedJobTypes.length > 0 && !selectedJobTypes.includes(job.type?.toLowerCase() ?? '')) {
          return false;
        }

        // Experience level filter
        if (experienceLevel && job?.experienceLevel?.toLowerCase() !== experienceLevel.toLowerCase()) {
          return false;
        }
        ...
        // Work type filter
        if (workType && job.jobType?.toLowerCase() !== workType.toLowerCase()) {
          return false;
        }
        ...
        if (selectedSkills.length > 0 && job.requiredSkills) {
          const jobSkills = Array.isArray(job.requiredSkills) 
            ...
            : job.requiredSkills.split(',').map((s: string) => s.trim().toLowerCase());
  ...
  if (error) {
    return (
      <div className="flex items-center gap-2 text-red-500">
        ...
        <span>{error instanceof Error ? error.message : 'Failed to fetch jobs'}</span>
      </div>
    );
  }
